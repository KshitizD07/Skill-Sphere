...
export function errorMiddleware(err, req, res, next) {
  ...
  // Prisma known errors
  if (err.code === 'P2002') return res.status(409).json({ error: 'DUPLICATE_ENTRY',  message: 'Record already exists' });
  if (err.code === 'P2025') return res.status(404).json({ error: 'NOT_FOUND',        message: 'Record not found' });
  if (err.code?.startsWith('P')) return res.status(500).json({ error: 'DATABASE_ERROR', message: 'Database operation failed' });

  // JWT errors (defensive — normally caught in middleware)
  if (err.name === 'JsonWebTokenError') return res.status(401).json({ error: 'INVALID_TOKEN', message: 'Invalid token' });
  if (err.name === 'TokenExpiredError') return res.status(401).json({ error: 'TOKEN_EXPIRED',  message: 'Session expired' });
